import React from 'react'
    ...
    const NextLaunches = ({ launches }) => {
      if(launches && Object.keys(launches).length !== 0){
        return (
          <div className="container" style={{maxWidth: '97%', padding: '0px'}}>
            <h2 style={{borderStyle: 'solid', borderWidth: '0px 0px 1px 0px'}}>Next Launches</h2>
            <div className="row" style={{marginLeft: '3px', lineHeight: '2px'}}>
            {launches.map((launch) => (
              <div className="col-md-6 card" key={launches.indexOf(launch)} style={{marginBottom: '1px', background:' #eeeeee', paddingTop: '10px'}}>
